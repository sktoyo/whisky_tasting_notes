...
#### 상단: 키워드 선택 영역
- **기본 키워드 목록 표시**
  - 기본 제공 키워드(VocabularyTerm): **Flavor Wheel 기반 키워드 계층 구조 (한국어)**
    - 1단계: 대분류 (과일향, 꽃향, 단맛, 견과류, 향신료, 고소한)
    - 2단계: 중분류 (베리류, 시트러스, 바닐라, 꿀, 코코아, 시나몬 등)
    - 3단계: 세부 키워드 (딸기, 레몬, 사과, 캐러멜, 아몬드, 민트 등)
    - UI는 단계적 확장 방식
      - **대분류 버튼 클릭** → 세부분류 영역 펼침 (키워드 자동 추가 없음)
      - 세부분류 영역 최상단에 **"대분류 키워드" 버튼** 표시
      - 대분류 키워드 버튼 클릭 → 대분류가 키워드로 추가됨
      - 중분류별 그룹 표시 → 각 중분류 칩 + 해당 세부 키워드 칩들
    - 적합한 세부 키워드를 모르겠으면 중분류 또는 대분류 키워드 선택 가능
    - 모든 키워드는 **이모지 아이콘**과 함께 칩(chip) 형태로 표시
...
- **키워드 선택/해제**
  - 키워드 클릭 시 선택/해제 토글
  - 선택된 키워드는 배경색 변경으로 시각적 표시 (amber 색상)
  - 이벤트 위임 방식으로 동적 추가된 키워드도 클릭 가능
- **커스텀 키워드 추가**
  - `+ 커스텀 키워드 추가` 버튼 클릭 시 모달 팝업
  - 새 키워드 입력 및 저장
...
### G-1. Flavor Wheel 계층 구조
- **데이터 소스**: `docs/flavor_category.json`
- **계층 구조**:
  - 대분류(6개): FRUITY, FLORAL, SWEET, NUTTY, SPICY, SAVORY
  - 중분류(각 대분류당 4개): BERRY, CITRUS, VANILLA, HONEY 등
  - 세부 키워드(각 중분류당 4개): STRAWBERRY, LEMON, APPLE 등
- **한국어 번역**: 모든 키워드는 한국어로 저장 및 표시
- **아이콘**: icon_key에 이모지가 직접 저장됨 (레거시 매핑 없음)
...
### G-3. 데이터베이스 제약사항
- **UniqueConstraint**: `(scope, term, level)` 조합이 unique
- 같은 이름의 키워드가 다른 level에 존재 가능
  - 예: "견과류" (level=1, 대분류)와 "견과류" (level=2, 중분류)
  - 예: "다크 초콜릿" (level=2, 중분류)와 "다크 초콜릿" (level=3, 세부 키워드)
